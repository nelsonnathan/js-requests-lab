...
// CODE HERE
function originalBtnColor () {
    sayHelloButton.style.backgroundColor = "#EFEFEF"
    sayHelloButton.style.color = "black"
}

sayHelloButton.addEventListener('mouseout', originalBtnColor)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/hmmm`) 
        .then(res => {
            console.log(res.data)
...
function queryTestFunc() {
    axios.get(`http://localhost:3000/query-test?name=nate&life=42`)
        .then(res => {
            console.log(res.data)
        })
        .catch(err => console.log(err))
...
document.getElementById('query-button').addEventListener('click', queryTestFunc)
...
//1. console logs the message "You sent an empty query!"

//2. console logs the message "You sent more than 1 query!"
